...
- **호출 기전**: `_shared/backends.json` **정본** (D11 — 호출 기전 값을 여기 재기술하지 않는다). 확인: `jq '.workers["codex-main"]' _shared/backends.json`
  - 오케스트레이터가 채우는 호출별 값은 `cwd` 하나:
    - 기본: `<설치한-폴더>/tasks/<task>/` — 이 안에서 산출물·diff 직접 작성
    - 외부 쓰기 4조건 충족 시: brief.md의 `target_repo` 값으로 변경
  - ※ sandbox는 cwd 내부만 쓰기 허용. cwd 밖은 sandbox가 차단 (값 자체는 backends.json)
...
- **파일 쓰기**:
  - 기본: cwd=`tasks/<task>/` → 작업 폴더 내부 산출물·diff 직접 작성 가능 (외부 repo는 sandbox가 막음. sandbox 값은 backends.json 정본)
...
- **결과물**: 비평 리스트, 수정 제안
- **호출 기전**: `_shared/backends.json` **정본** (D11). codex-main과 같은 MCP 도구를 쓰되 **read-only**로 분리된 별도 엔트리 — 확인: `jq '.workers["codex-critic"]' _shared/backends.json`
  - brief에 "비평 모드" 명시
