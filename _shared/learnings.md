...
## [2026-07-18] [clipper-pr1-merge · 통합 리뷰]

**웨이브별 리뷰가 전부 GO여도 누적 diff 통합 리뷰는 별도 가치를 가진다.** clipper의 두 웨이브(rebrand·lang-enko)가 각자 critic 리뷰를 통과하고 "인니어 저장소 전체 0"까지 선언했으나, 머지 직전 누적 92파일 통합 리뷰가 **NO-GO 3건**을 잡았다. 최대 지적은 `clipper_core.py`의 인도네시아어 403/봇탐지 오류 메시지 3블록이 `raise Exception`으로 **사용자에게 실제 노출**되던 것 — lang-enko의 "인니어 0" 선언과 정면 충돌. 원인은 그 웨이브의 ground truth 검증이 **렌더 덤프**(시작 화면)를 기준 삼아 "처리 중·실패 시에만 뜨는 예외 경로 문자열"을 원리적으로 못 본 것(그 세션이 자인한 맹점의 4번째 사례). **경계면 결함**(rebrand가 지운 걸 lang-enko 카탈로그가 되살렸나 등)은 웨이브 안에서는 보이지 않는다 → 머지 게이트에 통합 리뷰 1회를 넣을 값어치가 실증됐다.
